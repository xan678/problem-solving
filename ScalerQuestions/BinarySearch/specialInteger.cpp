...
bool checkFunc(vector<int> &A, int B, long k){
    long subSum = 0;
    for( long i =0 ; i<k;i++){
        subSum += A[(int)i];
    }
    ...
    for(long i = k; i<A.size();i++){

        subSum = subSum - A[(int)(i-k)] + A[(int)i];
